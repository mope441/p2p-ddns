...
use iroh::SecretKey;
use p2p_ddns::{
    admin::{authz::ClientRegistry, handler, protocol::ClientCommand},
    cli::args::DaemonArgs,
    net::init_network,
    storage::Storage,
};
...
#[tokio::test]
async fn set_service_propagates_to_joined_peer_promptly() -> Result<()> {
    let dir_a = tempfile::tempdir()?;
    let dir_b = tempfile::tempdir()?;
    ...
    let outcome = handler::handle_command(
        &ClientCommand::SetService {
            name: "openclaw-agent".to_string(),
            port: 39092,
        },
        &ctx_b,
        &clients,
    ...
    assert!(matches!(
        outcome.response,
        p2p_ddns::admin::protocol::ClientResponse::Ack(_)
    ));

    tokio::time::timeout(Duration::from_secs(2), async {
        loop {
            let has_service = ctx_a
                .nodes
                .get(&id_b)
                .and_then(|node| node.services.get("openclaw-agent").copied())
                == Some(39092);
            if has_service {
                break;
            }
